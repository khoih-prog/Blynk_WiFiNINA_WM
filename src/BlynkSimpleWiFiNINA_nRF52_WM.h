...
public:
    ...
    bool connectWiFi(const char* ssid, const char* pass)
    {
        ...
        WN_LOGERROR1(("WiFiNINA Firmware Version: "), WiFi.firmwareVersion());

        // attempt to connect to Wifi network:
        while (status != WL_CONNECTED) 
        {
            WN_LOGERROR1(("Con2:"), ssid);
            if (pass && strlen(pass)) 
            {
...
#define RETRY_TIMES_CONNECT_WIFI			3
      ...
      {
#if ( BLYNK_WM_DEBUG > 1)      
        WN_LOGERROR(("Double Reset Detected"));
#endif        
  ...
  // Force range of user-defined TIMES_BEFORE_RESET between 2-100
      ...
      if ( !wifi_connected || !connected() )
      {
        ...
        else
        {
          ...
          if ( configuration_mode && (configTimeout != 0) )
          {
            ...
            {
              WN_LOGERROR1(("r:Wlost&TOut.ConW+B.Retry#"), retryTimes);
            }
            else
            {
          ...
          if ( !wifi_connected )
          {
            WN_LOGERROR(("r:WLost.ReconW+B"));

            if (connectMultiWiFi(RETRY_TIMES_RECONNECT_WIFI))
            {
              WN_LOGERROR(("r:WOK.TryB"));
          ...
          //WN_LOGERROR(("run: Lost connection => configMode"));
          //startConfigurationMode();
        }
      }
    ...
    String getLocalIP()
    {
      ...
      //WN_LOGERROR1(("getLocalIP: IP = "), ipAddress);
      return ipAddress;
    }
...
#define RFC952_HOSTNAME_MAXLEN      24
    ...
    {
      WN_LOGERROR3(("Hdr="),       configData.header,
                 (",BrdName="),  configData.board_name);
      WN_LOGERROR3(("SSID="),      configData.WiFi_Creds[0].wifi_ssid,
                 (",PW="),       configData.WiFi_Creds[0].wifi_pw);
      WN_LOGERROR3(("SSID1="),     configData.WiFi_Creds[1].wifi_ssid,
                 (",PW1="),      configData.WiFi_Creds[1].wifi_pw);
      WN_LOGERROR3(("Server="),    configData.Blynk_Creds[0].blynk_server,
                 (",Token="),    configData.Blynk_Creds[0].blynk_token);
      WN_LOGERROR3(("Server1="),   configData.Blynk_Creds[1].blynk_server,
                 (",Token1="),   configData.Blynk_Creds[1].blynk_token);
      WN_LOGERROR1(("Port="),      configData.blynk_port);
      WN_LOGERROR(("======= End Config Data ======="));
      ...
      WN_LOGERROR(("======= End Dynamic Data ======="));
    }

    void displayWiFiData()
    {
      WN_LOGERROR5(("IP = "), IPAddressToString(WiFi.localIP()), (", GW = "), IPAddressToString(WiFi.gatewayIP()),
                 (", SN = "), IPAddressToString(WiFi.subnetMask()));
    }
...
#define BLYNK_CONNECT_TIMEOUT_MS      5000L
      ...
      for (int i = 0; i < NUM_BLYNK_CREDENTIALS; i++)
      {
        ...
        {
          WN_LOGERROR3(("Con2BlynkServer="), Blynk_WM_config.Blynk_Creds[i].blynk_server,
                     (",Token="), Blynk_WM_config.Blynk_Creds[i].blynk_token);
      ...
      WN_LOGERROR(("Blynk not connected"));
  ...
  #define CONFIG_TIMEOUT			60000L
    ...
    void startConfigurationMode()
    {
      ...
      WN_LOGERROR5(("stConf:SSID="), portal_ssid, (",PW="), portal_pass, (",IP="), portal_apIP);
